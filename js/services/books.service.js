...
function addReview(bookId, reivew) {
	reviews.push({ id: utilService.makeId(4), bookId: bookId, review: reivew });
	utilService.saveToStorage(REVIEW_KEY, reviews);
}

function getReviews(bookId) {
	const reviews = utilService.loadFromStorage(REVIEW_KEY);
	if (!reviews) return [];
	console.log(reviews);
...
function getCurrSymbol(name) {
	var currSymbol = '';
	for (const symbol in _symbols) {
		if (symbol === name) {
			currSymbol = _symbols[symbol];
		}
	}
	return currSymbol;
}
